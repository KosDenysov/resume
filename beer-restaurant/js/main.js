...
class ItcSimpleSlider {
    ...
    static EL_WRAPPER = `${ItcSimpleSlider.PREFIX}__container`;
  ...
  const toggleSelectors = document.querySelectorAll(".menu-toggle-selector");
  const toggleTabs = document.querySelectorAll(".menu-items");

  let tabToggle = (e) => {
    e.preventDefault();
    tab = 0;
    document.querySelector(".menu-toggle-selector-active").classList.remove("menu-toggle-selector-active");
    document.querySelector(".menu-items-active").classList.remove("menu-items-active");
    e.target.classList.add("menu-toggle-selector-active");
    tab = +e.target.dataset.tab;
    toggleTabs[tab].classList.add("menu-items-active");
  }

  for (let i=0; i<toggleSelectors.length; i++) {
    toggleSelectors[i].addEventListener("click", tabToggle);
  }
  ...
  let elements = document.querySelectorAll('.animate-right');
  let elementsL = document.querySelectorAll('.animate-left');
